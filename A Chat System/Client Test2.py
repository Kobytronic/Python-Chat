...
SERVER = "10.185.73.138"                                                         #This sets where the Server actually is and where to connect too
...
class Client():
        ...
        if self.alive:
            ...
            for events in pygame.event.get():
                if events.type == pygame.QUIT:
                    pygame.quit()
            count = 0
            for currentnumber, message in enumerate(chatlog):
                if message == "'":
                    self.flipflop = True
                    count += 1
                if self.flipflop == True:
                    ...
                    screen.blit(messageimg, (30, currentnumber*44+67))
    ...
    def clientstart(self):
        clientsend = threading.Thread(target=self.usermsg)                    #Create a thread
        clientrecv = threading.Thread(target=self.recvmsg)
        self.clientsendflip = True
        self.clientrecvflip = True 
        self.usernameflip = True
        while self.alive:
            clock.tick(60)
            self.screendisplay()
            ...
            if self.clientsendflip:
                clientsend.start()                                                  #make them run
                self.clientsendflip = False
            if clientsend.is_alive() != True:
                self.clientsendflip = True
            if self.clientrecvflip:
                clientrecv.start()                                                  #Another one
                self.clientrecvflip = False
            if clientrecv.is_alive() != True:
                self.clientrecvflip = True
