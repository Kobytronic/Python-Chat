...
class Client():
    def __init__(self):
        ...
        self.recvmsgflip = True
                                                                                #
    def send_msg(self, msg):                                                    #Functionnssssss
        if self.alive:                                                        #If you can talk
            ...
            if msg == DISCONNECT_MESSAGE:                                       #If the message is the disconnect message
                self.alive = False                                            #Don't allow Further message sending
    ...
    def recvmsg(self):                                                          #Receive messages
        if self.alive:                                                        #if you can talk
            ...
            if msg_length:                                                      #if it's correct                                    #get the length of the message as an integer
        ...
        while self.alive:
            ...
            for events in pygame.event.get():
                if events.type == pygame.QUIT:
                    self.alive = False
                if events.type == pygame.K_a and clientsend.is_alive() != True:
                    print("Hello?")
                    clientsend = threading.Thread(target=self.usermsg)
                    clientsend.start()
